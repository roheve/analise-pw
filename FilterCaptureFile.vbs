...
'	RH	28-07-2011	v0.06	Added commands, detected with firmware upgrade
...
' Make shure to set the MaxOutputBytes high enough (I choose 192)  
'
...
'
' The filter looks for *_READ or *_WRITE tags and extracts the message
'
CONST PW_CMD_0000 = "0000"	'* general response code
...
CONST PW_CMD_000A = "000A"	'* PlugWise System Initialisation Command
...
Dim strMACsrc		' Usualy the circle+ (NC)  
...
'read and filter data
	...
	'* READ
		bRead = TRUE
		'* skip rest of the line, parse next line in READ mode
	ElseIf bRead Then
	'* Finish readin READ data	
		intLengthPos=Instr(strLine,"Length")
		If intLengthPos>0 Then
			...
			intStartPos = Instr(intLengthPos,strLine,":")
			strLine= LTrim(Mid(strLine,intStartPos+1))
		'* 	Wscript.Echo "DEBUG" & vbTab & "SourceLine:" & intLine & vbTAB & strLine
...
'			Wscript.Echo "CRC - Succes MSG " &strCRCmsg & " - Calc "& strCRCcalc  
		...
		Case "000A"			'* Sort of reset/initialise
		...
		Case "0013"	'* RSP-Actual Powerusage - Reply
			...
			DecodeCommand = strCommand & " " & strResult '  & vbTab & "{" & HexToFloat(strPwrPulseCum) & "}"
		...
		Case "0040"	'* Command, configure schema settings (expect 0000 reply with 00E7 flag)
			strMACsrc 	= Mid(strOption,1,16)	: strResult = strMACsrc 
		...
		Case "004A"	'* Set Broadcast interval
			...
			strBcInterval	= Mid(strOption,17,2)	: strResult = strResult & " " & strBcInterval 
			strBcEnable	= Mid(strOption,19,2)	: strResult = strResult & " " & strBcEnable
			DecodeCommand = strCommand & " " & strMACsrc & " " & strBcInterval & " " & strBcEnable
		Case "0056"	'* Set/Create Group (reply from 0047) 
			...
			DecodeCommand = strCommand & " " & strMACsrc & " " & strBcInterval & " " & strBcEnable & " " & mid(strOption,23)
		Case "0057"	'* CMD0057 - Set something
			strMACsrc 	= Mid(strOption,1,16)	: strResult = strMACsrc 
			strUnknown1	= Mid(strOption,21,4)	: strResult = strResult & " " & strUnknown1 
			strUnknown2	= Mid(strOption,25,4)	: strResult = strResult & " " & strUnknown2
			DecodeCommand = strCommand & " " & strResult
